function BikeTrackingWithEDnew() %#codegen
...
%vid       = '320x240clip.avi';%'vippedtracking.avi';
...
obj = setupSystemObjects(vid, scaleDataFile);
...
%option.ROI                  = [425,100,200,650];   % A rectangle [x, y, w, h] that limits the processing area to ground locations.
%option.ROI                  = [100,200,200,500];   % A rectangle [x, y, w, h] that limits the processing area to ground locations.
option.ROI = [150,55,100,155];
...
option.timeWindowSize       = 5;               % A tuning parameter to specify the number of frames required to stabilize the confidence score of a track.
option.confidenceThresh     = 2;                % A threshold to determine if a track is true positive or false alarm.
option.ageThresh            = 11;                % A threshold to determine the minimum length required for a track being true positive.
...
function obj = setupSystemObjects(vid,scaleDataFile)
       ...
       obj.vid = videoinput('winvideo', 1,'MJPG_320x240');
